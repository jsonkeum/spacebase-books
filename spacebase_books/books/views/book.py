    ...
    def form_valid(self, form):
        try:
            ...
            if 'book_id' in self.kwargs:
                book, book_created = Book.objects.update_or_create(pk=self.kwargs['book_id'], defaults={
                    "title": data["title"],
                    "author": data["author"]
                })
            else:
                book, book_created = Book.objects.get_or_create(title=data["title"], author=data["author"])
            BookUser.objects.update_or_create(user=self.request.user, book=book, defaults={
                "rating": data["rating"],
            ...
            context['error'] = constants.EXTERNAL_ID_ERROR if 'external_id' in str(e) else str(e)
...
class EditBookView(BookFormView):
    def get(self, request, *args, **kwargs):
        ...
        book_user = get_object_or_404(BookUser, user=request.user, book_id=kwargs['book_id'])
...
def export_to_csv(request):
    ...
    response[
        'Content-Disposition'] = f'attachment; filename="books-{request.user.username}-{datetime.datetime.now()}.csv"'

    writer = csv.writer(response)
    writer.writerow(["Book ID", "External ID", "Title", "Author", "Rating"])
    for book in user_books:
        details = book.get_user_detail(request.user)
        writer.writerow([book.id, details.external_id, book.title, book.author, details.rating])
